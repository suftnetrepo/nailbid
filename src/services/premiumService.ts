 ...
 * premiumService.ts
 *
 * Manages NailBid Pro entitlement and purchases via RevenueCat.
 *
 * ─── Gating model ──────────────────────────────────────────────────────────
 * NailBid Pro does not cap usage. The single gate is the watermark on
 * exported PDFs (see pdfService.ts) — free-tier exports carry a visible
 * "Made with NailBid" banner, Pro removes it.
 *
 * ─── Setup still needed before this can go live ────────────────────────────
 * - iOS is wired up: real products + RevenueCat entitlement/offering + API
 *   key are all in place.
 * - Android still needs the equivalent: products in Google Play Console,
 *   mirrored in RevenueCat, and ANDROID_API_KEY below replaced with the
 *   real "goog_" key.
...
import { PREMIUM_STORAGE_KEY, PREMIUM_ENTITLEMENT_ID } from '../constants/premium'
...
const IOS_API_KEY     = 'appl_NwVTKIRKWScZQvADWidxuAMttjv'
